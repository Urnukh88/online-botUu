...
export const Sidebar = ({
  ...
  return (
    <aside
      className={`bg-[#161616] border-r border-white/5 flex flex-col h-screen relative z-20 transition-all duration-300 ease-in-out bg-gray-100 border-black dark:bg-black ${
    ...
    >
      <div className="p-6">
        ...
        >
          ...
          <span className="text-sm font-light tracking-widest uppercase text-slate-700 dark:text-slate-300 group-hover:text-[#C5A059] transition-colors">
            New Chat
          </span>
        </button>
      </div>
        ...
        {history.length === 0 ? (
          ...
          history.map((chat) => (
            ...
            >
              ...
              <span className="text-sm font-light text-slate-700 dark:text-slate-400 group-hover:text-slate-200 transition-colors truncate">
                {chat.title}
              </span>
            </button>
          ))
