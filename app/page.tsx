...
const SplineScene = dynamic(
  ...
  {
    ssr: false,
    loading: () => <div className="h-125 w-full bg-white/5 animate-pulse rounded-3xl" />
  }
);
...
const buyProduct = async (productName: string, productPrice?: string | number) => {
  ...
  try {
    const response = await fetch("/api/chat", {
      ...
      body: JSON.stringify({ 
        messages: [...currentChatMessages, { role: "user", content: userMsg }] 
      }),
    });
  ...
  const handleScroll = () => {
    if (messagesContainerRef.current) {
      ...
      setCanScrollRight(
        ...
        messagesContainerRef.current.scrollWidth - messagesContainerRef.current.clientWidth
      );
    }
  };
    ...
    <div className="flex-1 flex flex-col min-w-0 h-screen relative">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        <SparklesCore
          ...
          particleColor={typeof window !== 'undefined' && document.documentElement.classList.contains('dark') ? "#0A84FF" : "#007AFF"}
          speed={1}
        />
      </div>
              ...
              >
                ...
                p: ({ children }) => {
                  ...
                  const hasImage = childrenArray.some(
                    (child: any) => child.type === 'img' || (child.props && child.props.src)
                  );

                  if (hasImage) {
                    const images = childrenArray.filter((child: any) => child.type === 'img' || (child.props && child.props.src));
                    const textOnly = childrenArray.filter((child: any) => child.type !== 'img' && !(child.props && child.props.src));
                  ...
                  return <p className="mb-4 leading-relaxed text-slate-700 dark:text-slate-200">{children}</p>;
                },
                img: ({ ...props }) => {
                ...
                return (
                    <div 
                      onClick={() => setSelectedProduct({ name, price, image: props.src })}
      ...
      <ChatInput onMessageReceived={handleNewMessage} history={currentChatMessages} setIsTyping={setIsTyping} />
    </div>
    </div>
  );
}